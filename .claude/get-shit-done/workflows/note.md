...
- **Global scope**: `/Users/mateuspires/Dev/agent-clinic/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/mateuspires/Dev/agent-clinic/.claude/notes/`)
...
2. Glob `/Users/mateuspires/Dev/agent-clinic/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/mateuspires/Dev/agent-clinic/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/mateuspires/Dev/agent-clinic/.claude/notes/` — works in any directory
