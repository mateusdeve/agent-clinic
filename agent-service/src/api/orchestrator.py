...
async def _flush_buffer(
    ...
    async with _phone_locks[phone]:
        ...
        # Emit typing_indicator: Sofia iniciou processamento (D-02)
...
async def _process_message(
    ...
    # Broadcast bot response via Socket.IO para panel viewers (WPP-04)
    clean_phone = phone.split("@")[0]
    try:
        from src.api.socketio_server import broadcast_new_message, broadcast_typing_indicator
        from datetime import datetime
        for msg in mensagens:
            conteudo = msg.get("conteudo", "") if isinstance(msg, dict) else str(msg)
            if conteudo:
        ...
        # Typing indicator: Sofia concluiu (D-02)
